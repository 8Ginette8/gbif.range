...
tax <- get_status("Cypripedium calceolus", children = TRUE, related = FALSE)
tax
```

When `children = TRUE`, the output is deliberately close to the logic used by `get_gbif()`: it returns the accepted name and the synonyms that are actually used for occurrence retrieval, i.e., including subspecies and variety. While FALSE is only exploratory and strictly keeps species level names. When `related` = FALSE`, additional related names are also included for taxonomic inspection, but those extra names are not used to download occurrences.
