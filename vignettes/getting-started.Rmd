...
The function originates from the occurrence retrieval workflow first introduced
...
⚠️ Note that the download takes longer without `occ_samp`. Although giving
...
subsample was retrieved, the resulting map closely follows the GBIF sampling
pattern. Increasing or removing `occ_samp` would produce a more complete
...
character column for `ecoreg_name`. Similarly, `make_ecoreg()` accepts any
spatially structured raster, not just climate layers. See Part 1 for full
details on both.
...
restricted or biogeo-constrained distribution, these parameters often need
...
(*GBIF Retrieval, Taxonomy, and Filtering*) covers `get_status()`, `get_gbif_count()`,
`get_gbif()`, `obs_filter()`, `make_tiles()` and `get_doi()`. Part 2
...
`evaluate_range()`.Part 3 (*Large Downloaded GBIF Tables*) describes the
disk-based batch workflow built around `split_gbif_by_species()`,
