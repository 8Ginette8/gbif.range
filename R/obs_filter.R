...
#' of records.
#'
#' @param gbifs Output from `get_gbif()` including one or several species. Note
...
#' @param threshold Optional integer specifying the minimum number of observations
...
obs_filter <- function(gbifs, grid, threshold = NULL) {
  ...
  out.sp <- lapply(n.sp, function(x) {
    ...
    new_oxy <- data.frame(Species = x, terra::xyFromCell(grid, unique_posP))

    # Apply threshold filtering if specified
    if (!is.null(threshold)) {
      cell_counts <- table(posP)
      filtered_posP <- unique_posP[unique_posP %in% names(cell_counts[cell_counts >= threshold])]
      new_oxy <- new_oxy[terra::cellFromXY(grid, new_oxy[, c("x", "y")]) %in% filtered_posP, ]
    }
